...
- **About page** – showroom story, quick stats, address, phone, email, and
  opening hours.
- **Login & Register** – users can create an account (passwords are stored as
  bcrypt hashes) and log in. The session uses a JWT token, and the navbar
  shows the logged-in user's name with a Logout button.
...
| Other | CORS, dotenv, bcryptjs (password hashing), JWT (jsonwebtoken) |
...
│   │   ├── pages/            # Home, Cars, Booking, MyBookings, About, Login, Register
...
└── backend/                  # Express + MongoDB API
    ├── models/               # Car.js, Booking.js, User.js (Mongoose schemas)
    ├── routes/               # cars.js, bookings.js, auth.js
...
- Login/registration use **JWT tokens** saved in localStorage – fine for a
  demo, but a real website would use httpOnly cookies and link each booking
...
- Link bookings to the logged-in user and show only their own bookings
